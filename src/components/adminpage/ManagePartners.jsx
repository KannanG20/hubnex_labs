...
const ManagePartners = () => {
  ...
  const handleSubmit = async ()=>{
    try {
      ...
      const res = await fetch(`http://${import.meta.env.VITE_API_URL}/api/v1/partner`,requestOptions )
  ...
  return (
    <div className='text-white'>
      ...
      <div className='flex justify-between items-center py-2 border-b-2 border-b-white'>
        ...
        <button onClick={()=> {
          setInvalidImg(false)
          setAddImg(true)}} className=' bg-blue-500 py-2 px-5 rounded-full'>Add Partner</button>
      </div>
      {addImg && 
